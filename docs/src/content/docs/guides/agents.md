...
A `recover_custody` action is ordinary `no-mistakes axi sync --recover` to take a still-available preserved head, or `no-mistakes axi sync --recover --keep-local` in the narrower cases where only keeping the current local head can return custody; run the exact reported command and never substitute one for the other, because each variant is offered only for the states it can actually resolve. [`no-mistakes axi sync`](/no-mistakes/reference/cli/#no-mistakes-axi-sync) owns the full case list. See [`no-mistakes rerun`](/no-mistakes/reference/cli/#no-mistakes-rerun) for the alternative validation path and its refusal conditions.
