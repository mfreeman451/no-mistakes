...
When the owning run ended without publishing its pipeline commits, the same box offers `u recover custody` instead: `u` opens a confirmation naming the terminal status, the local head, and the preserved head, and applying routes through the guarded recovery documented in [`no-mistakes axi sync`](/no-mistakes/reference/cli/#no-mistakes-axi-sync). `u` always runs exactly the recovery that plan offers, including its keep-local variant: when the structured proof is a bound archive for divergent later work, the confirmation also names the archive and required head and the archived head is never selected, and when the local gate branch diverged from the local head, the confirmation explains that recovery leaves the worktree untouched and moves the gate branch to the kept local head. If the preserved head is unavailable, the box instead tells you to keep the current local head; complete that missing-head recovery through the AXI path documented in the linked reference, because the TUI action does not discard a missing head.
