...
Returned custody must also be usable, so those cells check the gate's own branch ref: if the local head does not descend from it, the next `no-mistakes axi run` push is rejected non-fast-forward, which a run that rewrote history in its detached worktree can cause. Status then reports `safety: blocked_pipeline_owned_recoverable` with the command `no-mistakes axi sync --recover --keep-local`, and plain `--recover` refuses with `safety: blocked_recover_gate_diverged` rather than stamping a custody return it cannot deliver. The keep-local action leaves the worktree untouched, anchors the replaced gate head under `refs/no-mistakes/recover-gate/<run>`, and atomically points the gate branch at the kept local head.
...
Adoption also moves the gate's own branch ref onto the adopted head, because a run that rebased in its detached gate worktree and died before pushing leaves that shared ref at the pre-rebase commit while the rewritten head survives only at the run recovery ref: taking the rewritten head locally and leaving the gate behind stamps a custody return whose very next `no-mistakes axi run` push is rejected non-fast-forward. The gate move is decided before anything is touched and accepts only three heads: the adopted head itself (nothing moves), an ancestor of it (an ordinary fast-forward), or the exact pre-recovery local head the containment proof was computed against. That last move is not a fast-forward, so the replaced head is anchored under `refs/no-mistakes/recover-gate/<run>` first and the swap is an atomic compare-and-swap pinned to the observed gate head, so a concurrent gate push wins and recovery refuses with `safety: blocked_recover_gate_race`. Any other gate head is evidence recovery cannot explain: it refuses with `safety: blocked_recover_gate_unexpected_head` and manual-reconciliation guidance rather than rewinding the shared ref. A custody stamp that fails after the gate already moved rolls the gate branch back to the exact head it replaced.
