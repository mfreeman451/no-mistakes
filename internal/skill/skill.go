...
When ` + "`next_action.code`" + ` is ` + "`recover_custody`" + `, run its exact ` + "`next_action.command`" + ` rather than reconstructing one. That is ` + "`no-mistakes axi sync --recover`" + ` to take a still-available preserved pipeline head, or ` + "`no-mistakes axi sync --recover --keep-local`" + ` in three keep-local cases: when an accessible gate confirms the verified preserved head is missing and you are explicitly discarding those unpublished commits, when a bound archive proves divergent later work remains preserved while recovery keeps the branch at the exact reported required head and never selects, merges, or replays the archive, or when the preserved head is already reachable locally but the gate branch diverged from your head, so only keep-local can move that gate branch and make the branch pushable again. Do not substitute plain ` + "`--recover`" + ` or ` + "`rerun`" + ` for a reported keep-local action. ` + "`no-mistakes rerun`" + ` can resume validating a still-available ordinary preserved head instead, subject to the clean-head check above.
...
Adoption also moves the local gate branch onto the adopted head, anchoring the head it replaces under ` + "`refs/no-mistakes/recover-gate/<run>`" + `, so a run that rebased and died before pushing cannot leave you with a branch whose next run is rejected non-fast-forward; a gate head recovery cannot account for refuses instead.
